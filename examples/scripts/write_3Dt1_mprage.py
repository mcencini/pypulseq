...
def main():
    ...
    # =========
    delta_kz = 1 / fov_z
    phase_areas = (np.arange(Ny) - (Ny / 2)) / Ny
    slice_areas = (np.arange(Nz) - (Nz / 2)) / Nz
    
    phase_area = Ny * delta_k
    slice_area = Ny * delta_kz
    
    gx_pre = pp.make_trapezoid(channel='x', system=system, area=-gx.area / 2, duration=2e-3)
    
    _gy_pre = pp.make_trapezoid(channel='y', system=system, area=phase_area, duration=2e-3)
    ...
    _gz_pre = pp.make_trapezoid(channel='z', system=system, area=slice_area, duration=2e-3)
    
    for i in range(Ny):
        gy_pre = pp.scale_grad(_gy_pre, scale=phase_areas[i])
        ...
        for j in range(Nz):
            gz_pre = pp.scale_grad(_gz_pre, scale=slice_areas[j])
            gz_reph = pp.scale_grad(_gz_pre, scale=-slice_areas[j])    
            seq.add_block(rf)
