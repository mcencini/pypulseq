...
def main(plot: bool = False, write_seq: bool = False, seq_filename: str = 'tse_pypulseq.seq'):
    ...
    # Excitation slice selection grad
    # gs1 = gz_slc_exc_ramp_up
    # gs2 = gz_slc_exc_flat
    # gs3 = gz_slc_exc_flat_to_slc_refoc_flat
    
    #######################################################################
    #######################################################################
    flip_ex = np.deg2rad(90.0)
    rf_ex, _gz_slc_exc, _ = pp.make_sinc_pulse(
        flip_angle=flip_ex,
        system=system,
        duration=t_ex,
        slice_thickness=slice_thickness,
    ...
    # Re-design the slice selection gradients to account
    # for rf ringdown and deadtime in flat portion
    gz_slc_refoc = pp.make_trapezoid(
        channel='z',
        system=system,
        amplitude=_gz_slc_exc.amplitude,
        flat_time=t_ex + system.rf_ringdown_time + system.rf_dead_time,
        rise_time=dG,
    )
    
    # 
    
    # Refocusing slice selection grad
    # gs4 = gz_slc_refoc_flat
    # gs5 = gz_slc_refoc_flat_to_spoil
    # gs6 does not exists - numbering is in sync with grN
    # gs7 = gz_spoil_to_slc_refoc_flat
    
    # Readout
    # gr1 does not exists - numbering is in sync with gsN
    # gr2 does not exists - numbering is in sync with gsN
    # gr3 = gx_spoil - brings to gx_spoil.area = kx_start
    # gr4 does not exists - numbering is in sync with gsN
    # gr5 = gx_read_generalized_pre - brings it from -kx_start to -kx_edge = -(Nx / FOVx)
    # gr6 = gx_read - brings from -kx_edge to +kx_edge
    # gr7 = gx_read_generalized_rew - time reversed gx_read_generalized_pre, brings from kx_edge to kx_start
    
    # ======
    # CONSTRUCT SEQUENCE
    # ======
    for excitation_idx in range(n_ex + 1):
        for slice_idx in range(n_slices):
            # Slice-select frequency and phase settings for excitation
            rf_ex.freq_offset = gz_ex.amplitude * slice_thickness * (slice_idx - (n_slices - 1) / 2)
            rf_ref.freq_offset = gs_ref.amplitude * slice_thickness * (slice_idx - (n_slices - 1) / 2)
            
            # Slice-select frequency and phase settings for refocus
            ...
            # --- Excitation block sequence ---
            seq.add_block(gz_slc_exc_ramp_up)
            seq.add_block(rf_ex, gz_slc_exc_flat)
            seq.add_block(gx_spoil, gz_slc_exc_flat_to_slc_refoc_flat) # this is an optimized spoiler along x and z
    
            # --- Echo train ------
            for echo_idx in range(n_echo):
                if excitation_idx > 0:
                    phase_encode_area = phase_encode_table[echo_idx, excitation_idx - 1] 
                else:
                    phase_encode_area = 0.0  # 0.0 and not 0 because -phase_area should successfully result in negative zero
                
                # Phase encoding and rewinder
                gy_phase_encode_pre = pp.make_trapezoid(
                    channel='y',
                    system=system,
                    area=phase_encode_area,
                    duration=t_sp,
                    rise_time=dG,
                )
                gy_phase_encode_rew = pp.make_trapezoid(
                    channel='y',
                    system=system,
                    area=-phase_encode_area,
                    duration=t_sp,
                ...
                # --- Refocusing block  ---
                seq.add_block(rf_ref, gz_slc_refoc_flat)
                
                # --- Readout ----
                seq.add_block(gx_read_generalized_pre, gy_phase_encode_pre, gz_slc_refoc_flat_to_spoil) # Prewind
                if excitation_idx > 0:
                    seq.add_block(gx_read_flat, adc) # Readout
                else:
                    seq.add_block(gx_read_flat) # Dummy Readout
                seq.add_block(gx_read_generalized_rew, gy_phase_encode_rew, gz_spoil_to_slc_refoc_flat) # Rewind
            
            # ---- Post Echo Train (Final Spoil and Reset to initial k-space position) ----
            seq.add_block(gz_slc_refoc_flat)
            seq.add_block(gz_slc_refoc_flat_to_spoil)
